...
def NextStep(vector_12, vector_9,gripper_state,write_file):
    ...
    '''#this block of code was used to generate the test csv file
    i = 1
    while i < 100:
...
def TrajectoryGenerator(
    ...
    segment1 = CartesianTrajectory(initial_ee_config, initial_standoff_config, Tf, N, 5)
    tj_csv(segment1, csv, 0, "a")

    """Segment 2 : moving from initial standoff to grasping position"""
    segment2 = CartesianTrajectory(initial_standoff_config, grasp_config, Tf, N, 5)
    tj_csv(segment2, csv, 0, "a")

    """Segment3 : closing the gripper"""
    grip_state_change(csv, True)

    """Segment4 : moving back to standoff position"""
    Segment4 = CartesianTrajectory(grasp_config, initial_standoff_config, Tf, N, 5)
    tj_csv(Segment4, csv, 1, "a")

    """Segment5 : moving to final standoff config"""
    Segment5 = CartesianTrajectory(
        initial_standoff_config, final_standoff_config, Tf, N, 5
    )
    tj_csv(Segment5, csv, 1, "a")

    """Segment6 : moving cube to its final config"""
    segment6 = CartesianTrajectory(final_standoff_config, final_cube_config, Tf, N, 5)
    tj_csv(segment6, csv, 1, "a")

    """Segment7: Opening the gripper"""
    grip_state_change(csv, False)

    """Segment8 : Back to final standoff config"""
    Segment8 = CartesianTrajectory(final_cube_config, final_standoff_config, Tf, N, 5)
    tj_csv(Segment8, csv, 0, "a")
...
def FeedbackControl(current_config, desired_config, next_config, Kp, Ki, timestep):
    dsrd_twist = (
        MatLog6(np.matmul(TInv(desired_config), next_config))
        * 1
        / timestep
    )
    dsrd_twist = Se3ToV(dsrd_twist)
    adjoint = Adjoint(np.matmul(TInv(current_config), desired_config))
    err_config = MatLog6(np.matmul(TInv(current_config), desired_config))
    V = Se3ToV(err_config)
...
def T0_e(last_line,home_cfg,Blist):
    ...
    T0e= FKB(home_cfg,Blist,theta_list)
    return T0e
...
def jacobian(last_line,B_list,Home_config):
    ...
    Tb0 = np.array([[1, 0, 0, 0.1662], [0, 1, 0, 0], [0, 0, 1, 0.0026], [0, 0, 0, 1]])
    J_base = Adjoint(TInv(T0e) @ TInv(Tb0)) @F6
    theta_list=np.array(last_line[3:8]).T
    J_body= JB(B_list,theta_list)
...
if __name__ == "__main__":
    ...
    Tscini = np.array(
        [[1, 0, 0, 1.5], [0, 1, 0, 0], [0, 0, 1, 0.025], [0, 0, 0, 1]]
    )  # initial cube configuration
    Tscdes = np.array(
       [[0.044, 0.998, -0.041, 0], [0.726, -0.060, -0.685, -1.5], [-0.686, 0, -0.727, 0.025], [0, 0, 0, 1]]
    )  # final cube configuration  z = 0.025

    T_grasp = np.array([[-0.888,0,0.461,1.5],[0,1,0,0],[-0.461,0,-0.888,0.025],[0,0,0,1]])

    Tscini_standoff = np.array([[-0.888,0,0.461,1.5],[0,1,0,0],[-0.461,0,-0.888,0.070],[0,0,0,1]])

    Tscdes_standoff =   np.array(
        [[0.044, 0.998, -0.041, 0], [0.726, -0.060, -0.685, -1.5], [-0.686, 0, -0.727, 0.070], [0, 0, 0, 1]]
    ) 
    ...
    traj_csv = "Trajectory.csv"    
    KP = np.eye(6) * 1
    KI = np.eye(6) * 1
    cfg = [phi,x,y,j1,j2,j3,j4,j5,w1,w2,w3,w4]
    controls = [ 0,0,0,0,0,0,0,0,0]
    cfg_file = open(config_csv,"w")
    NextStep(cfg,controls,0,True)
    curr_cfg = act_curr_config(T0_e(last_line(config_csv),M0e_home,B_list),last_line(config_csv))
    traj_file = open(traj_csv,"r")
    tj_r= traj_file.readlines()                                               #csv_curr_cfg
    dsrd_cfg = tj_r[0]
    g_s=dsrd_cfg[-2]
    dsrd_cfg = csv_to_ndarray(dsrd_cfg)
    nxt_cfg = tj_r[1]
    nxt_cfg = csv_to_ndarray(nxt_cfg)
    J = jacobian(last_line(config_csv),B_list,M0e_home)
    test_joint_limits(csv_curr_cfg(config_csv),g_s,J,FeedbackControl(curr_cfg,dsrd_cfg,nxt_cfg,KP,KI,0.01))
    curr_cfg = act_curr_config(T0_e(last_line(config_csv),M0e_home,B_list),last_line(config_csv))
